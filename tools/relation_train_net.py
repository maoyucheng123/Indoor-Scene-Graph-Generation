...
def train(cfg, logger, args):
    available_metrics = {"mR": "_mean_recall",
                         "R": "_recall",
                         "zR": "_zeroshot_recall",
    ...
    best_epoch = 0
    best_metric = 0.0
    best_checkpoint = None  # Initialize best_checkpoint to None
    ...
    print("\n======== Relation Feature Extractor (Union) ========\n")
    print(model.roi_heads.relation.union_feature_extractor)

    print("\n======== MDFA Module inside Relation Feature Extractor ========\n")
    print(model.roi_heads.relation.union_feature_extractor.mdfa)
    ...
    pretrain_mask = (
                cfg.MODEL.ROI_RELATION_HEAD.SQUAT_MODULE.PRETRAIN_MASK and cfg.MODEL.ROI_RELATION_HEAD.PREDICTOR == "SquatPredictor")
    if pretrain_mask:
    ...
    for epoch in range(0, max_epoch):
        ...
        _ = train_one_epoch(model, optimizer, train_data_loader, device, epoch, logger, cfg, scaler, args['use_wandb'],
                            use_amp)
        ...
        if current_metric > best_metric:
            ...
            if args['save_best']:
                ...
                best_checkpoint = os.path.join(cfg.OUTPUT_DIR,
                                               "best_model_epoch_{}".format(epoch))  # Update best_checkpoint
                if to_remove is not None:
                    os.remove(to_remove + ".pth")
                    logger.info("New best model saved at epoch {}".format(epoch))

        logger.info(
            "Best Epoch in {} is : {}, with value {}".format(cfg.METRIC_TO_TRACK + "@k", best_epoch, best_metric))
